...
class DailyStatusPortalController extends Controller
{
    ...
    public function getDataPortalClassic()
    {
    ...
    public function getDataPortalAtak()
    {
    ...
    public function getPortalClassicHtml()
    {
        ...
        $xml = $this->getHtmlComparativeReport($link, $viewsState, $evanValidation);
        return $xml;
    }
    ...
    public function getHtmlComparativeReport($link, $viewsState, $evanValidation)
    {
        $client = new Client([
...
//        $dataPortal = $this->getDataInPortalClassic();
        $dataPortal1 = $this->getDataPortalClassic();
        $dataPortal2 = $this->getDataPortalAtak();

        $citeName = $dataPortal1['citeName'];
        $clientsValue = $dataPortal1['clientsValue'];
        $CaPeriodValue = $dataPortal1['CaPeriodValue'];

        $citeName2 = $dataPortal2['citeName'];
        $clientsValue2 = $dataPortal2['clientsValue'];
        $CaPeriodValue2 = $dataPortal2['CaPeriodValue'];
        ...
        foreach ($citeName2 as $key => $value1) {

            foreach ($clientsValue2 as $key2 => $value2) {
                if ($key2 == $key) {
                    $allCiteArr2[$value1]['client_value'] = $value2;
                }
            }
            ...
            foreach ($CaPeriodValue2 as $key5 => $value5) {
                if ($key5 == $key) {
                    $allCiteArr2[$value1]['CA_period_value'] = $value5;
                }
            }
        };
        ...
        foreach ($allCiteArr2 as $key => $value) {
            foreach ($value as $key2 => $value2) {
                // условие для закрытых магазов где нет данных
                if ($key2 == 'CA_period_value' && $value2 == '*****') {
                    $problemCiteArr2[$key] = 'Close';

                    // условие для открытых магазов где нет данных
                } elseif ($value2 == '*****') {
                    $problemCiteArr2[$key] = 'No data';
                }
            }
        }
