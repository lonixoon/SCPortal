    ...
    $.extend($.datepick, {
        ...
        selectWeek: function (picker, inst) {
            ...
            picker.find('td.datepick-week span,td.ui-state-hover span').each(function () {
                var t = $('<a href="javascript:void(0)" class="' +
                ...
                t.click(function (event) {
				    ...
				    if (inst.get('rangeSelect')) {
				        ...
				        for (var i = 1; i < 7; i++) {
                            dates.push(tmp = $.datepick.add($.datepick.newDate(tmp), 1, 'd'));
                        }

                        var minDate;
                        if (inst.pickingRange == true && ((new Date(inst.selectedDates[0])).getTime() < (new Date(date)).getTime())) {
                        ...
                        if (inst.pickingRange == true && ((new Date(tmp)).getTime() < (new Date(inst.selectedDates[1])).getTime())) {
                            maxDate = inst.selectedDates[1];
                        } else {
                            maxDate = tmp;
                        }
                        ...
                        inst.selectedDates[0] = dates[0];
				            inst.selectedDates[1] = dates[dates.length - 1];
				            inst.selectedDates = [inst.selectedDates[0], inst.selectedDates[1]];

                        if (inst.pickingRange) {
				            dates = [inst.selectedDates[0], inst.selectedDates[1]];
				            target.datepick('setDate', dates).datepick('hide');
				        }
				        else {
				            target.datepick('setDate', inst.selectedDates).datepick('hide');
